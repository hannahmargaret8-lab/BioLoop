...
def run_paired_timepoint(
    ...
    """
    Run zero-bias EIS first, followed by a +0.1 V to +0.8 V
    DC-bias sweep in 0.1 V increments.
    """
...
def run_biofilm_dc_pump_experiment():
    ...
    log(
        folder,
        f"Pincher routing: MCP GP{PINCHER_MCP_PIN} ON for "
        f"the first {SEED_HOURS} hr, then OFF for the "
    ...
    log(
        folder,
        f"Scans per voltage condition: {EIS_SCANS_PER_MODE}; "
        f"conditions per timepoint: {1 + len(DC_BIAS_VALUES_V)} "
        "(zero bias plus DC sweep)",
    )
    log(
        folder,
        "DC-bias sweep: "
        + ", ".join(f"{bias:+.3f} V" for bias in DC_BIAS_VALUES_V),
    )
    log(
        folder,
    ...
    try:
        ...
        # Power the pincher from the start so the fluid path is
        # directed toward the seeding/waste route for the first
        # three hours.
        pincher.set_state(PINCHER_ON_DURING_SEEDING)
        log(
            folder,
            f"Pincher MCP GP{PINCHER_MCP_PIN} ON: "
            "seeding/waste route selected",
        )
        ...
        input(
            "\nLoad sterile LB/media into the chip and confirm "
            "the chamber is bubble-free.\n"
            "Confirm MCP GP13 is powered and the seeding/waste "
            "route is selected.\n"
            "Press Enter to run paired media-baseline EIS..."
        )
        ...
        run_paired_timepoint(
            ...
            elapsed_hr=0.0,
            notes=(
                "Sterile media before bacterial seeding; "
                "pincher ON; seeding/waste route selected"
            ),
        )

        input(
            "\nSeed bacteria into the SPE chamber.\n"
            "Confirm the pump remains OFF, then press Enter "
            "to begin the three-hour static attachment period..."
        )

        seed_start = datetime.now()
        log(folder, "Static bacterial attachment started")

        completed_wait = countdown_wait(
            total_seconds=SEED_HOURS * 3600,
            folder=folder,
            next_event="post-seed paired EIS",
        )

        if not completed_wait:
            return

        # Three-hour routing transition:
        # de-energize MCP GP13 before the post-seed measurement.
        # The valve remains OFF for all subsequent media-flow timepoints.
        pincher.set_state(PINCHER_OFF_AFTER_SEEDING)
        log(
            folder,
            f"Three-hour transition: pincher MCP GP"
            f"{PINCHER_MCP_PIN} OFF; "
        ...
        run_paired_timepoint(
            ...
            elapsed_hr=elapsed_hr,
            notes=(
                "After static bacterial attachment; "
        ...
        log(folder, "Biofilm DC-bias sweep experiment finished")